...
The cf CLI command `cf push` pushes apps to <AppRuntimeAbbr/>. There are two main ways to run the `cf push` command:
...
For an explanation of what <AppRuntimeAbbr/> does when you run `cf push`, see [How Apps are
...
Before you push your app to <AppRuntimeAbbr/>, ensure that:

* Your app is "cloud-ready." <AppRuntimeAbbr/> behaviors related to file storage, HTTP sessions, and port usage might require modifications to your app. To prepare an app to be pushed to <AppRuntimeAbbr/>, see:
...
* Your <AppRuntimeAbbr/> deployment supports the type of app you are going to push, or you have the URL of an externally-available buildpack that can stage the app.
...
* You have your target and credentials:
  * The API endpoint for your <AppRuntimeAbbr/> deployment. Also known as the target URL, this is <APIEndpoint/>.
  * Your user name and password for the <AppRuntimeAbbr/> deployment.
...
1. Choose a name for the app. The app name must consist of alphanumeric characters and be unique to your <AppRuntimeAbbr/> deployment.
...
An app's route is the URL at which it runs. <AppRuntimeAbbr/> assembles the route for a pushed app from a host name and a domain.

By default, <AppRuntimeAbbr/> sets the host name and domain as follows:

* **Hostname:** The name of the app name, as specified in the `cf push` command. If the app name contains underscores, <AppRuntimeAbbr/> converts them to hyphens when creating the app's route.

* **Domain:** The default apps domain for your <AppRuntimeAbbr/> deployment.

For example, an app named `example-app` running on <AppRuntimeAbbr/> with an apps domain `apps.example.com` runs at the URL
...
The following example output for `cf push example-app` demonstrates how <AppRuntimeAbbr/> assigns default values to an app you push:
...
* **Instances:** Generally speaking, the more app instances you run, the less downtime your app experiences. If your app is still in development, running a single instance can simplify troubleshooting. For any production app, <RecommendedBy/> recommends a minimum of two instances.
...
<AppRuntimeAbbr/> restarts the instance. If an app instance exceeds its memory limit repeatedly in a short period of time,
<AppRuntimeAbbr/> delays restarting the app instance.
...
<AppRuntimeAbbr/> restarts the instance. If an app instance exceeds its disk space limit repeatedly in a short period of time,
<AppRuntimeAbbr/> delays restarting the app instance.
...
<AppRuntimeAbbr/> drops the excess logs and reports doing so.

* **Start command:** This is the command that <AppRuntimeAbbr/> uses to start each instance of your app. This start command differs by app framework.

### <a id='exclude'></a> Limit the upload files (optional)

By default, <AppRuntimeAbbr/> uploads all app files except version control files and directories with names such as `.svn`, `.git`, and `_darcs`.

<RecommendedBy/> recommends that you explicitly exclude extraneous files residing within your app directory, particularly if your app is large. For example, if you build your app locally and push it as a binary, you can save resources by not uploading any of the app's source code.
...
These tasks run after <AppRuntimeAbbr/> loads the app droplet but before it starts the app to allow the initialization script to access the app language runtime environment. For example, your script can map values from `$VCAP_SERVICES` into other environment variables or a config file that the app uses.
...
The following notes include important information about configuring app initialization when you use certain buildpacks:
  <ul>
    ...
    *Java:* Initialization scripts for the Java buildpack require additional configuration.
    {config.platform_code != 'CF' ? (<>For more information, see <a href="https://community.pivotal.io/s/article/how-to-modify-the-application-container-environment-prior-to-application-execution?language=en_US">How to Modify the Application Container Environment prior to Application Execution</a> in the VMware Tanzu Knowledge Base.</>) : null}
    </li>
    <li>
      *PHP:* <AppRuntimeAbbr/> does not support initialization scripts for the PHP buildpack versions prior to v4.3.18. If you use
...
By default, when you push an app that is already running, <AppRuntimeAbbr/> stops all existing instances of that app. Users who try to access the app see a `404 Not Found` message while `cf push` runs.
...
If your app does not start on <AppRuntimeAbbr/>, first ensure that the app can run locally.
